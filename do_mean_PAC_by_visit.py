...
targ_folder = 'new_pac_' + channelstr + '_AVG_' + str(start_seg) + '_' + str(end_seg) + '_NOBORDER'
        ...
        if len(vinfo)>0:
            ...
            if len(svisits)>0:
                ...
                for vo in visit_order:
                    ...
                    # pac_avg = pd.DataFrame((np.zeros((224,224))), columns=lbl_224, index=lbl_224)
                    ...
                    for f in seg_fn:
                        ...
                        # grayImage.show()
                        array = np.array(grayImage) 
                        row_lbl = [str(i) for i in np.arange(0,array.shape[0])]
                        col_lbl = [str(i) for i in np.arange(0,array.shape[1])]
                        this_seg = pd.DataFrame(array, columns=col_lbl, index=row_lbl)
