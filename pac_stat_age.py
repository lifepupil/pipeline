    ...
    for age_rng in age_groups:
        ...
        for sex in SEX: 
            ...
            for targ_folder in datasets:
                ...
                # for s in range(0,len(pd_filtered)-1): 
                #     fname = pd_filtered.iloc[s].eeg_file_name + '.jpg'
                #     if os.path.isfile(pth + fname):
                #         age = pd_filtered.iloc[s].age_this_visit

                #         # print(fname)
                #         if pd_filtered.iloc[s].AUD_this_visit==True:
                #             dx = 'alcoholic'
                #             labels_dx.append([1,age])
                #         else:
                #             dx = 'nonalcoholic'
                #             labels_dx.append([0,age])
                ...
                for s in range(0,len(pd_filtered_age_balanced)-1): 
                    ...
                    if os.path.isfile(pth + fname):
                        ...
                        id = pd_filtered_age_balanced.iloc[s].ID
                        ...
                        if pd_filtered_age_balanced.iloc[s].AUD_this_visit==True:
                            dx = 'alcoholic'
                            labels_dx.append({'ID' : id, 'age': age, 'yalc' : yalc, 'evd' : evd, 'afd' : afd, 'AUD' : 1})
                        else:
                            dx = 'nonalcoholic'
                            labels_dx.append({'ID' : id, 'age': age, 'yalc' : yalc, 'evd' : evd, 'afd' : afd, 'AUD' : 0})
                ...
                if 1:
                    ...
                    ttl = targ_folder + ' alc_' + str(len(ages_alc)) + ' unaff_' + str(len(ages_ctl)) 
                    ...
                    pac_age = pd.DataFrame({'PAC' : regions, 'alc' : labels_dx[:,0], 'age' : labels_dx[:,1]})

                    
                    pa_alc = pac_age[pac_age.alc==1]
                    pa_ctl = pac_age[pac_age.alc==0]

                    ax1 = pa_alc.plot(y='PAC',x='age', c='r', kind='scatter',label='AUD', s= 2, title=ttl + '\n' + reglbl)
                    # ax1.set_yscale('log')
                    pa_ctl.plot(y='PAC',x='age', c='b', kind='scatter',label='unaff', ax=ax1, s= 2)
                    plt.xlim([15,75])
                    plt.ylim([155,220])
                    plt.show()
                    ...
                    lgd = ['Unaff', 'AUD']
                    ax2 = pac_age[pac_age.alc==0].PAC.hist(bins=bins,legend=False, histtype='step', color='blue') #, alpha=1, edgecolor='blue', linewidth=1)
                    pac_age[pac_age.alc==1].PAC.hist(bins=bins, ax=ax2, alpha=0.4, color='red') #, edgecolor='red', linewidth=1)
                    plt.legend(lgd)
                    plt.title(ttl)
                    plt.xlabel('PAC')
                    plt.ylabel('Counts')
