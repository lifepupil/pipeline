# -*- coding: utf-8 -*-
"""
Created on Thu Jul 10 15:48:40 2025
...
Fs = ['gng_2_a1_p0000882_32.fif', 'gng_2_a1_a0001385_32.fif', 'gng_2_a1_40205010_32.cnt']
...
for F in Fs:
    ...
    for c in cols[3:]:
        ...
        for epoch_i in go_epoch_list:
            # segment = g[epoch_i]
            segment  = g.query('epoch==' + str(epoch_i)).values[0]
            title = 'Go trial channel ' + c + ' epoch ' + str(epoch_i)
            ...
            xpac = p.filterfit(sample_rate, segment, n_perm=n_perm, n_jobs=-1, random_state=0, verbose=None).squeeze()
