# -*- coding: utf-8 -*-
"""
Created on Wed Jul 23 16:42:00 2025
...
Fs = [
      'gng_2_a1_40205010_32_eeg.fif',
      'gng_2_a1_a0001385_32_eeg.fif', 
      'gng_2_a1_p0000882_32_eeg.fif'
      ]
...
for F in Fs:
    ...
    info = raw.info
    eeg_indices = mne.pick_types(info, meg=False, eeg=True, ecg=False, eog=False, exclude=['BLANK','X','Y']) #, 'AF7', 'AF8'])
    ...
    time = df[df.epoch==1].time.values
    ...
    for pa in range(len(freq_pairs)):
        ...
        for c in cols[3:]:
            ...
            gg = dfgo.pivot(index='epoch', columns='time',values=c)
            data = gg.to_numpy()
            ...
            fp_lbl = [( round(x[0],2) ) for x in p.f_pha]
            fp_i = np.arange(0,len(p.f_pha),1)
            fp_dict = dict(zip(fp_i,fp_lbl))
            binsphi = [round( np.mean([x[0],x[1]]),2) for x in p.f_pha]

            fa_lbl = [(x[0]) for x in p.f_amp]
            fa_i = np.arange(0,len(p.f_amp),1)
            fa_dict = dict(zip(fa_i,fa_lbl))
            binsamp = [round( np.mean([x[0],x[1]]),2) for x in p.f_amp]

            
            pha = p.filter(sample_rate, data, ftype='phase', n_jobs=1)
            amp = p.filter(sample_rate, data, ftype='amplitude', n_jobs=1)
            erpac = p.fit(pha, amp, method='circular', n_perm=n_perm, mcp='fdr', verbose=False).squeeze()
            ...
            pvalues = p.pvalues
            xamp, yphi, ztime = np.where(pvalues<0.05)

            amps = [fa_dict[i] for i in xamp]


            phis = [fp_dict[i] for i in yphi]
            
            minp = pvalues.min()
            print('channel ' + c + '  P-A frequency pair ' + str(this_fp) + '\t minimum p-value = ' + str(round(minp,5)) + '    N = ' + str(len(data)))
            ...
            if minp<=alpha:
                ...
                # ax[0].hist(xamp, bins=len(p.f_amp), label='significant amp histogram', align='mid')
                # ax[0].hist(xamp, bins=binsamp, label='significant amp histogram', align='mid', edgecolor="k", alpha=1)
                ax[0].hist(amps, bins=len(p.f_amp), label='significant amp histogram', align='mid', edgecolor="k", alpha=1)
                # ax[0].set_xticks([], minor=False)
                # ax[0].tick_params(axis='x', labelsize=6) 
                # ax[0].set_xticks(fa_i,fa_lbl)
                # ax[0].set_xticklabels(fa_lbl)
                ax[0].set_title(c + ' amp')
                # ax[1].hist(yphi, label='significant phase histogram', align='mid')
                ax[1].hist(phis, bins=len(p.f_pha), label='significant phase histogram', align='mid', edgecolor="k", alpha=1)
                # ax[1].hist(yphi, bins=binsphi, label='significant phase histogram', align='mid', edgecolor="k", alpha=1)
                # ax[1].set_xticks([], minor=False)
                # ax[1].tick_params(axis='x', labelsize=6) 
                # ax[1].set_xticks(fp_i,fp_lbl)
                # ax[1].set_xticklabels(fp_lbl)
                ax[1].set_xlabel(c + ' phase')
                plt.show()
                ...
                if not(np.isnan(vmin)):
                    ...
                    # for i in ([erpac.shape[0]-1]):
                        ...
                        title = c + ' ' + trial_type + ' ' + 'n_pha=' + str(fa_dict[i]) + ' ' + str(this_fp) + ' minp=' + str(round(minp,5))
                        p.pacplot(erpac[i, : , :], time, p.xvec, xlabel='Time (second)',
                                  cmap='Spectral_r', ylabel='Phase frequency', title=title,
